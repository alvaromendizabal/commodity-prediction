...
from commodity_prediction.runtime import RunLog, atomic_json, digest, fingerprint, seal_checkpoint, verify_checkpoint
...
def declared_comparisons() -> list[tuple[str, str]]:
    return [(v, c) for v in VARIANTS for c in ["admitted_tail", "historical_mean"]] + [
        ("joint_path", "current_market"), ("joint_path", "price_path"),
...
def study_lineage(root: Path) -> tuple[str, dict]:
    ...
    if config["parent_lineage"] != parent or config["max_new_fits"] != 12 or config["feature_gate"] != "open":
        raise ValueError("Unreviewed study configuration")
    files = {str(p.relative_to(root)): digest(p) for p in sorted(
        (root / "src/commodity_prediction/domain/market_path").glob("*.py"))}
    evidence = {"parent_lineage": parent, "feature_lineage": prior["feature_lineage"],
                "config": config, "files": files, "comparisons": declared_comparisons(),
...
def load_inputs(root: Path, feature_lineage: str):
    ...
    if len(x) != 1709 or not x.date_id.equals(y.date_id) or list(y.columns) != ["date_id", *targets]:
...
def run_study(root: Path, fold_limit: int = 1, sync: bool = False) -> dict:
    ...
    def timeout_handler(signum, frame):
        atomic_json(budget_path, {"elapsed_seconds": 300, "limit_seconds": 300, "deadline_reached": True})
    ...
    try:
        ...
        for stage, expected in [(feature_dir, evidence["feature_lineage"]), (parent_stage, evidence["parent_lineage"])]:
            ...
            for variant in VARIANTS:
                ...
                for fold in folds[:fold_limit]:
                    ...
                    with log.stage(f"fold_{fold.number}/{variant}"):
                        ...
                        if not complete:
                            new_fits += 1
                        result = fit_stage(panel, y, pairs, fold, Experiment(variant, algorithm="histogram"),
                                           settings, stats, stage, lineage)
                        if any(k in result["selection"]["rejection_reasons"] for k in ["feature_budget", "correlated", "unstable_sign"]):
                        ...
                        log.event("checkpoint_verified", completed=len(results), total=4 * fold_limit,
                                  variant=variant, fold=fold.number, reused=bool(complete),
                ...
                summaries[variant] = {**metrics, "candidate_templates": len(panel.names),
                                      "fold_scores": [r["metrics"]["official_metric"] for r in results if r["variant"] == variant]}
                del panel
                gc.collect()
            if fold_limit == 1:
                reference = parent["summaries"]["admitted_tail"]["fold_scores"][0]
                proceed = not all(v["official_metric"] < reference - 0.10 for v in summaries.values())
                probe = {"lineage": lineage, "continue_allowed": proceed, "results": results,
                         "summaries": summaries, "frozen_control_score": reference,
                         "fits_this_invocation": new_fits, "maximum_prediction_replay_error": replay_error,
                         "status": "first_fold_completed", "feature_gate": "open", "holdout_evaluated": False}
                atomic_json(directory / "probe.json", probe)
                atomic_json(root / "reports/market_path_probe.json", probe)
                budget()
                return probe
            for name in ["admitted_tail", "historical_mean", "screened_tail", "tail_states_and_priors", "short_own"]:
                summaries[name] = parent["summaries"][name]
            with log.stage("matched_joint_uncertainty"):
                daily = joint_history(root, parent)
                for prefix, filename in [("risk_state", "risk_state_study"), ("released_context", "released_context_study")]:
                    older = json.loads((root / "reports" / (filename + ".json")).read_text())
                    sealed = root / "artifacts" / older["lineage"] / "summary"
                    if not verify_checkpoint(sealed, older["lineage"]) or json.loads((sealed / "summary.json").read_text()) != older:
                        raise ValueError("Comparison history differs from private seal")
                    daily.update({prefix + "::" + n: np.asarray(s["daily_rank_correlations"]) for n, s in older["summaries"].items()})
                daily.update({"market_path::" + n: np.asarray(s["daily_rank_correlations"]) for n, s in summaries.items()})
                contrasts = list(dict.fromkeys((c["variant"], c["reference"]) for c in parent["joint_comparisons"]))
                contrasts += [("market_path::" + a, "market_path::" + b) for a, b in declared_comparisons()]
                bounds = compare_predictions(daily, lengths, contrasts, config)
                report = {"lineage": lineage, "parent_lineage": evidence["parent_lineage"],
                          "feature_lineage": evidence["feature_lineage"], "feature_gate": "open",
                          "holdout_evaluated": False, "validation_dates": 535,
                          "new_fitted_models": len(results), "fits_this_invocation": new_fits,
                          "model_checkpoints_replayed": len(results), "maximum_prediction_replay_error": replay_error,
                          "checkpoint_count": len(results) + 1, "inventories": inventories,
                          "summaries": summaries, "results": results, "joint_comparisons": bounds,
                          "joint_comparison_count": len(contrasts),
                          "comparisons": [c for c in bounds if c["variant"].startswith("market_path::")],
                          "limitations": ["Repeatedly inspected development folds; conditional intervals do not undo adaptation.",
...
def main() -> None:
    ...
    print(json.dumps({"status": result.get("status", "completed"), "lineage": result["lineage"],
                      "new_fitted_models": result.get("new_fitted_models", 4), "feature_gate": "open"}), flush=True)
