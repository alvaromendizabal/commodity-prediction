...
Eleven new tests passed for release boundaries, prefix equivalence, peer exclusion, signed orientation, missingness, partial reuse and tamper detection. New source, aggregate results and the completed models are fingerprinted independently of all previous studies. The canonical research notebook publication completed on September 10, 2026. The [AWS publication receipt](../reports/aws_released_context_publication.json) records nine independent model replays with maximum prediction difference 0.0, 589 verified stage manifests, one re-executed research notebook, 34 Plotly/static figure pairs across the three project notebooks, and zero new training fits during publication.
