...
path = Path("logs/robustness-publication.log")
...
if path.exists():
    ...
    s3.upload_file(str(path), settings["bucket"], "bootstrap/robustness-publication.log",
                   ExtraArgs={"Metadata": {"sha256": checksum}})
    status["log_sha256"] = checksum
s3.put_object(Bucket=settings["bucket"], Key="bootstrap/robustness-publication-status.json",
