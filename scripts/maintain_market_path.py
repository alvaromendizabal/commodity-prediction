...
def main():
    ...
    if (
        ...
        or report["new_fitted_models"] != 12
    ...
    block += (
        "Measured on the same 535 purged development origins. These are exploratory "
    ...
    for name, values in report["scores"].items():
        block += (
            ...
            + name
            + " | "
            + " | ".join(f"{v:.6f}" for v in [values["official_metric"], *values["fold_scores"]])
            + " |\n"
        )
    block += (
        "\n"
    ...
    nb["cells"] = [
        c for c in nb["cells"] if c.get("metadata", {}).get("market_path_evidence") is not True
    ]
    nb["cells"].append(
        {
            "cell_type": "markdown",
            "id": "market-path-evidence",
            "metadata": {"market_path_evidence": True},
    ...
    if header not in text:
        text = text.replace(
            ...
            + "\n\n"
            + report["decision"]
            + " See [measured evidence](reports/market_path_publication.json) and [feature protocol](docs/market-path-research.md).\n",
            1,
        )
